    ...
    <>
      ...
      <div className="min-h-screen bg-[#fff] pt-24 px-2 md:px-8">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="text-center text-gray-500 mb-10">Home &mdash; <span className="text-black font-semibold">Shopping Cart</span></div>
          {cart.length === 0 ? (
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 md:p-8">
              <div className="hidden md:grid grid-cols-12 gap-4 border-b pb-4 mb-4">
                ...
                <div className="col-span-2 font-bold text-lg text-center">Price</div>
                <div className="col-span-3 font-bold text-lg text-center">Quantity</div>
                <div className="col-span-2 font-bold text-lg text-right">Total</div>
              </div>
              <ul className="divide-y divide-gray-200">
                {cart.map((item, idx) => {
                  ...
                  const qty = item.qty || 1;
                  return (
                    <li key={idx} className="grid grid-cols-2 md:grid-cols-12 gap-4 items-center py-4">
                      {/* Product info */}
                      ...
                      {/* Price */}
                      <div className="hidden md:block col-span-2 text-center font-bold text-[#2D5A27]">₵{price.toFixed(2)}</div>
                      {/* Quantity controls */}
                      <div className="col-span-1 md:col-span-3 flex items-center justify-center gap-2">
                        <button onClick={() => handleQty(idx, -1)} className="w-8 h-8 rounded bg-gray-100 text-xl font-bold hover:bg-gray-200">-</button>
                        <span className="px-3 text-lg font-semibold">{qty}</span>
                        <button onClick={() => handleQty(idx, 1)} className="w-8 h-8 rounded bg-gray-100 text-xl font-bold hover:bg-gray-200">+</button>
                      </div>
                      {/* Total */}
                      <div className="col-span-1 md:col-span-2 text-right font-bold text-[#2D5A27]">₵{(price * qty).toFixed(2)}</div>
                      {/* Remove */}
                      <div className="col-span-1 flex justify-end">
                        <button onClick={() => handleRemove(idx)} className="text-gray-400 hover:text-red-500 text-2xl font-bold">&times;</button>
                      </div>
                    </li>
                  );
                })}
              ...
              <div className="flex flex-col md:flex-row justify-between items-center mt-8 gap-6">
                ...
                <div className="bg-gray-50 rounded-lg p-6 w-full md:w-1/3">
                  ...
                  <button className="w-full py-3 bg-[#8BC34A] text-white font-bold rounded hover:bg-[#689F38] transition">Proceed to Checkout</button>
                </div>
              </div>
            </div>
          )}
